...
export function createResponseAgent(character: Character, context: AgentConferenceContext): Agent {
  ...
  const phaseInstruction: Record<string, string> = {
    explore: `【現在のフェーズ：探索 — 論点を広げる】
- 他の参加者がまだ触れていない観点を、自分の専門領域から出してください。
- 直前の発言と同じ話題に乗っからず、別の切り口を出す。例：材料の話が続いていたら、設備制約、顧客要求、コスト、後工程、公差、納期、安全など別の観点を持ち出す。
- 「それに加えて、○○の観点も検討すべきだと思う」のように論点を増やす発言をする。`,
    converge: `【現在のフェーズ：収束 — 選択肢を絞る】
- 新しい話題を広げず、既出の論点を深掘りしてください。
- 選択肢の比較、トレードオフの整理、優先順位の提案を行う。
- 「つまり○○か△△かの判断になる」のように論点を絞り込む。
- 前の発言者と同じ結論を繰り返さず、新しい判断材料を追加する。`,
    decide: `【現在のフェーズ：決定 — 結論を出す】
- 「私は○○を推奨する。理由は〜」のように明確な立場を示す。
- 曖昧な表現は避け、具体的な判断・推奨を述べる。
- 他の参加者の意見も踏まえた上で、自分の専門からの最終的な判断を出す。`,
    summarize: `【現在のフェーズ：まとめ — 合意事項を確認する】
- 議論で決まったことと残った課題を確認してください。新しい論点は出さない。
- 「今回の議論で○○は合意できた。△△は次回の課題」のように整理する。
- 具体的なアクション（誰が何をいつまでに）があれば言及する。`,
  }
