 ...
 * 習熟レベル 4段階 — 問題解決軸
 *
 * ものづくりの QCDE（品質・コスト・納期・環境）に対して
 * どのレベルの問題解決ができるかで段階を定義する。
 *
 * Lv.1 基礎: 基本手順に従い定型作業を遂行できる（層1で判定）
 * Lv.2 実践: 標準的な問題を自力で特定・解決できる（層2-D 専門性レベルで判定）
 * Lv.3 応用: 非定型な問題にQCDEの観点から解決策を立案できる（層2-C 自発性で判定）
 * Lv.4 熟練: 複数領域を横断しQCDEの最適バランスで問題解決できる（層2-A,B で判定）
 */
